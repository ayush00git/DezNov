...
route.get('/myProfile', protectedRoute, async(req, res) => {
    try {
        const user = await User.findOne({ _id: req.user.userId });
        if(!user){
            return res.status(400).json({ messgae: 'User is not authenticated' });
        }
        return res.status(200).json({ user });
    } catch (error) {
        return res.status(500).json({ messgae: `Error: ${error}` });
    }
})
