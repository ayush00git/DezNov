...
const changePass = async(token) => {
    ...
    try {
        await transporter.sendMail({
            ...
            to: decoded.email,
            subject: "Reset password of your DezNov account",
            html: `
                <!DOCTYPE html>
                <html>
                <head>
                ...
                <div style="max-width: 600px; margin: 0 auto; background-color: #1A1B23; border-radius: 12px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.3);">
                    ...
                    <div style="background-color: #2A2D37; border-radius: 8px; padding: 20px; margin: 30px 0;">
                        ...
                        <strong style="color: #F79009;">Reset password link:</strong><br>
                        <a href="${resetUrl}" style="color: #2A9F8D; text-decoration: none; word-break: break-all; font-size: 13px;">
                            ${resetUrl}
                        </a>
                        </p>
                    </div>
        ...
        console.log(`${error}`);
        throw new Error(`While sending email`);
    }
}
