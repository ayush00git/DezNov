...
function Auth({ className = "" }) {
  ...
  const [loginState, loginDispatch] = useReducer(reducer, { property1: "default" });
  const [signupState, signupDispatch] = useReducer(reducer, { property1: "default" });
  const [exploreState, exploreDispatch] = useReducer(reducer, { property1: "default" });

  return (
    <div className={`flex flex-wrap border-2 border-[#849ADE] text-white sm:w-full lg:w-fit py-3 sm:py-4 px-3 sm:px-6 rounded-full gap-3 justify-center items-center sm:gap-5 bg-[rgba(49,61,94,0.9)] ${className}`}>
      {/* Login Button */}
      <div
        className={`w-fit flex items-center gap-2 sm:gap-[7px] cursor-pointer shadow-[5px_5px_8px_#00000040] px-3 sm:px-[18px] py-2 sm:py-[5px] h-[38px] rounded-[23px] relative ${
      ...
      >
        ...
        <div className="[font-family:'Poppins-Medium',Helvetica] w-fit tracking-[0] text-base sm:text-lg text-white font-medium leading-[normal] relative">
          Log In
        </div>
      </div>
      ...
      {/* Signup Button */}
      <div
        className={`border border-solid w-fit cursor-pointer flex items-center gap-1 sm:gap-1.5 shadow-[5px_5px_8px_#00000040] px-3 sm:px-[18px] py-2 sm:py-[5px] h-[38px] rounded-[23px] relative ${
      ...
      {/* Explore Button */}
      <div
        className={`border border-solid w-fit cursor-pointer flex items-center gap-1 sm:gap-1.5 shadow-[4px_4px_8px_#00000040] px-3 sm:px-[17px] py-2 sm:py-[5px] h-[38px] rounded-[23px] relative ${
